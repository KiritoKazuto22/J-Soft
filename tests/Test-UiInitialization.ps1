...
try {
    ...
    $oldChocolateyInstalled = $script:JSoft.ChocolateyStatus.IsInstalled
    $script:JSoft.WingetStatus.IsInstalled = $false
    $script:JSoft.WingetStatus.SourceAvailable = $false
    $script:JSoft.ChocolateyStatus.IsInstalled = $true
    $script:JSoft.Ui.WingetRadio.IsChecked = $false
    $script:JSoft.Ui.ChocolateyRadio.IsChecked = $true
    Update-JSoftSelectionUi
    if (-not $script:JSoft.Ui.InstallButton.IsEnabled) {
        throw "Chocolatey-Auswahl aktiviert den Installationsbutton nicht, wenn WinGet nicht verfügbar ist."
    }
    $script:JSoft.Ui.ChocolateyRadio.IsChecked = $false
    $script:JSoft.Ui.WingetRadio.IsChecked = $true
